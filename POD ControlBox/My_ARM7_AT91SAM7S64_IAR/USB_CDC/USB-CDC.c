...
//static void vUSBSendBuffer( portCHAR  cByte []);
/*------------------------------------------------------------*/
...
//static unsigned portCHAR HEADER_LENGTH = 3;
//static unsigned portCHAR START_BYTE    = 0xB1;
// eno
...
__ramfunc void vUSBCDCTask( void *pvParameters )
{
xISRStatus *pxMessage;
unsigned portLONG ulStatus;
unsigned portLONG ulRxBytes;
//unsigned portCHAR ucByte;
unsigned portCHAR ucByteBuf[64];
unsigned portCHAR pucByteRecUSB [64];
portBASE_TYPE xByte;
unsigned portCHAR nCounter;
	...
	for( ;; )
	{
		...
		if( eDriverState == eREADY_TO_SEND )// eno && ucControlState )
		{
			if( ( !(AT91C_BASE_UDP->UDP_CSR[ usbEND_POINT_2 ] & AT91C_UDP_TXPKTRDY) ) && uxQueueMessagesWaiting( xTxCDC ) )
			{
                            if( usbqReadIoMsg(ucByteBuf ))
			    {
				for( xByte = 0; ((xByte < 64) && (ucByteBuf[xByte] != 0)) ; xByte++ )
				{				
					//Got a byte to transmit.
				  AT91C_BASE_UDP->UDP_FDR[ usbEND_POINT_2 ] = ucByteBuf[xByte];
				}
			...
			while( AT91C_BASE_UDP->UDP_CSR[ usbEND_POINT_1 ] & (AT91C_UDP_RX_DATA_BK0 | AT91C_UDP_RX_DATA_BK1) )
			{
				...
				{
                                 // pucByteRecUSB[0] = ulRxBytes;

                                        for( nCounter = 0; nCounter < ulRxBytes; nCounter++)

					//while( ulRxBytes-- )
					{
                                          // legger i mottatte buffer
                                          pucByteRecUSB[nCounter] = AT91C_BASE_UDP->UDP_FDR[ usbEND_POINT_1 ];
...
unsigned int GetCurrentBank(void)
{
  return  uiCurrentBank;
}
...
__ramfunc static void prvProcessEndPoint0Interrupt( xISRStatus *pxMessage )
{
	...
	if( pxMessage->ulCSR0 & AT91C_UDP_TXCOMP )
	{
		...
		if( eDriverState == eJUST_GOT_CONFIG )
		{
			...
			portENTER_CRITICAL();
			{
				...
				usbCSR_SET_BIT( &ulTemp, AT91C_UDP_EPEDS | AT91C_UDP_EPTYPE_INT_OUT );
...
//       pxDeviceDescriptor[16] = 0x01;   // Sensorskap
         pxDeviceDescriptor[16] =CalculateUnitID_fromADC(); //m_nUnitID; //
...
__ramfunc static void prvHandleStandardDeviceRequest( xUSB_REQUEST *pxRequest )
{
...
//	AT91C_BASE_PIOA->PIO_PER = AT91C_PIO_PA16;  // enable pin 16
//	AT91C_BASE_PIOA->PIO_OER = AT91C_PIO_PA16;  // pin 16 is output

;
	/* Disable pull up */
//	AT91C_BASE_PIOA->PIO_SODR = AT91C_PIO_PA16;   // set high pin 16
}
/*-----------------------------------------------------------*/
...
//extern void ( vUSB_ISR )( void );
    ...
    //    xRxCDC = xQueueCreate( 5, 64 );
	//xRxCDC = xQueueCreate( USB_CDC_QUEUE_SIZE, ( unsigned portBASE_TYPE ) sizeof( signed portCHAR ) );
	//xTxCDC = xQueueCreate( USB_CDC_QUEUE_SIZE + 1, ( unsigned portBASE_TYPE ) sizeof( signed portCHAR ) );// eno( unsigned portCHAR ) sizeof( signed portCHAR ) );
   //     xTxCDC = xQueueCreate( 5, 64 );

	if( !xUSBInterruptQueue)
	{	
	...
	/* Setup the PIO for the USB pull up resistor. */
//	AT91C_BASE_PIOA->PIO_PER = AT91C_PIO_PA16; // enable
//	AT91C_BASE_PIOA->PIO_OER = AT91C_PIO_PA16; // output


	/* Start without the pullup - this will get set at the end of this
	function. */
//   	AT91C_BASE_PIOA->PIO_SODR = AT91C_PIO_PA16; // high
	...
	// eno AT91F_AIC_ConfigureIt( AT91C_ID_UDP, usbINTERRUPT_PRIORITY, AT91C_AIC_SRCTYPE_INT_HIGH_LEVEL, ( void (*)( void ) ) vUSB_ISR );
	AT91C_BASE_AIC->AIC_IECR = 0x1 << AT91C_ID_UDP;


	/* Wait a short while before making our presence known. */
	vTaskDelay( usbINIT_DELAY );
//	AT91C_BASE_PIOA->PIO_CODR = AT91C_PIO_PA16;  // low pin 16
}
/*-----------------------------------------------------------*/
...

{
        ...
        pxDeviceDescriptor[16] = 0x02;   // HER SETTER JEG ID TIL 0x02 og her vi skulle kalle på funksjon som henter

                                                   // resultat av analog måling. F.eks.:

     // pxDeviceDescriptor[16] = GetID();     hvor "GetID" utfører analog måling og beregner ID
