...
static char cfgDescriptor[] = {
	/* ============== CONFIGURATION 1 =========== */
	/* Configuration 1 descriptor */
	0x09,   // CbLength
	0x02,   // CbDescriptorType
	0x20,//0x43,   // CwTotalLength 2 EP + Control
	0x00,
...
//static uint AT91F_UDP_Write(AT91PS_CDC pCdc,  char *pData, uint length);
static void AT91F_CDC_Enumerate(AT91PS_CDC pCdc);
__ramfunc void AT91F_USB_SendZlp(AT91PS_UDP pUdp);
__ramfunc void AT91F_USB_SendStall(AT91PS_UDP pUdp);


extern unsigned int GetCurrentBank(void);
//*----------------------------------------------------------------------------
//* \fn    AT91F_CDC_Open
//* \brief
//*----------------------------------------------------------------------------
 AT91PS_CDC AT91F_CDC_Open(AT91PS_CDC pCdc, AT91PS_UDP pUdp)
{
	...
	pCdc->currentRcvBank       = GetCurrentBank(); //AT91C_UDP_RX_DATA_BK1;
...
//*----------------------------------------------------------------------------
__ramfunc static uchar AT91F_UDP_IsConfigured(AT91PS_CDC pCdc)
{
...
//*----------------------------------------------------------------------------
__ramfunc uint AT91F_UDP_Read(AT91PS_CDC pCdc, char *pData, uint length)
{
	AT91PS_UDP pUdp = pCdc->pUdp;
	...
	while (length) {
		//if ( !AT91F_UDP_IsConfigured(pCdc) )
			//break;
		if ( pUdp->UDP_CSR[AT91C_EP_OUT] & (AT91C_UDP_RX_DATA_BK0 | AT91C_UDP_RX_DATA_BK1))//currentReceiveBank )
                {
                    ///////////////////////////////////////////////////////////
                    // correction if wrong receiving bank
			...
			pUdp->UDP_CSR[AT91C_EP_OUT] &= ~(currentReceiveBank); // clear receiving flag for this bank
			if (currentReceiveBank == AT91C_UDP_RX_DATA_BK0)      // change receiving bank
				currentReceiveBank = AT91C_UDP_RX_DATA_BK1;
			else
				currentReceiveBank = AT91C_UDP_RX_DATA_BK0;
...
//*----------------------------------------------------------------------------
 __ramfunc uint AT91F_UDP_Write(AT91PS_CDC pCdc, char *pData, uint length)
 //  static uint AT91F_UDP_Write(AT91PS_CDC pCdc, const char *pData, uint length)
{
	AT91PS_UDP pUdp = pCdc->pUdp;
	uint cpt = 0;
	...
	length -= cpt;
        ...
        while (pUdp->UDP_CSR[AT91C_EP_IN] & AT91C_UDP_TXPKTRDY); // wait to fifo is ready
        
	while (cpt--) pUdp->UDP_FDR[AT91C_EP_IN] = *pData++;  // write to fifo
	...
	while (length) 
		...
		while (pUdp->UDP_CSR[AT91C_EP_IN] & AT91C_UDP_TXCOMP); // to be shoure that TXCOMP has been cleared
		pUdp->UDP_CSR[AT91C_EP_IN] |= AT91C_UDP_TXPKTRDY;
	}
...
__ramfunc  static void AT91F_USB_SendData(AT91PS_UDP pUdp,  char *pData, uint length)
//__ramfunc  static void AT91F_USB_SendData(AT91PS_UDP pUdp, const char *pData, uint length)
{
	uint cpt = 0;
	AT91_REG csr;
...
//*----------------------------------------------------------------------------
 __ramfunc static void AT91F_CDC_Enumerate(AT91PS_CDC pCdc)
{
	AT91PS_UDP pUDP = pCdc->pUdp;
